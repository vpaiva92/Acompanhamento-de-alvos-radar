function [estado_posterior] = modelo_alvo_w_posterior(estado_anterior,T)

    % PARÂMETROS DO ALVO
    tau = estado_anterior(3);
    alfa = 1/tau;
    beta = exp(-alfa*T);
    ...
    w = beta*estado_anterior(6) + normrnd(0,sigma_w);
    ...
    if abs(w) > 0.05
        ...
        A = [1  T               0   0   -(w*T^2)/2      0   ;
             0  1-((w*T)^2)/2   0   0   -(w*T)          0   ;
             0  0               0   0   0               0   ;
             0  (w*T^2)/2       0   1   T               0   ;
             0  (w*T)           0   0   1-((w*T)^2)/2   0   ;
             0  0               0   0   0               beta];
    end
    ...
    ruido_de_processo = [normrnd(0,sigma_a) ;abs(492.996*exp(-0.138*estado_anterior(6))+normrnd(0,10)) ;normrnd(0,sigma_a) ;normrnd(0,sigma_w)];

    estado_posterior = A*estado_anterior + B*ruido_de_processo;
